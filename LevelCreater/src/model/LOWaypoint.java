...
public class LOWaypoint extends LOCircle {
	...
	public LOWaypoint(int xPos, int yPos, int xSize, int ySize) {
		...
		this.ellipse = new Ellipse2D.Double(xPos - xSize / 2, yPos - ySize / 2,
				xSize, ySize);
	}

	public LOWaypoint(boolean randomPosition, int totalNumOfWayPoints,
			int currentWayPointNum, double xSize, double ySize, int levelWidth,
			int levelHeight) {
		this.objectColor = Color.YELLOW;
		// this.position = new Point((int) (Math.random() * (levelWidth - 1)),
		// (int) (Math.random() * (levelHeight - 1)));
		this.position = getEvenlyDistributedWayPoint(totalNumOfWayPoints,
				currentWayPointNum, levelWidth, levelHeight);
		this.ellipse = new Ellipse2D.Double(this.position.x, this.position.y,
				xSize, ySize);
	}

	private Point getEvenlyDistributedWayPoint(int totalNumOfWayPoints,
			int currentWayPointNum, int levelWidth, int levelHeight) {
		...
		// |5|6|
		// -----
		// depending on width and length of level, favor vertical or horizontal
		// splitting
		...
		if (oneDirectionSplitting) {
			if (verticalSplittingFavored) {
				xStart = 0;
				xEnd = levelWidth;
				yStart = currentWayPointNum
						* (levelHeight / totalNumOfWayPoints);
				yEnd = (currentWayPointNum + 1)
						* (levelHeight / totalNumOfWayPoints);
			} else {
				xStart = currentWayPointNum
						* (levelWidth / totalNumOfWayPoints);
				xEnd = (currentWayPointNum + 1)
						* (levelWidth / totalNumOfWayPoints);
				yStart = 0;
				yEnd = levelWidth;
			}
			...
			yStart = (currentWayPointNum / (ySplit + faktor))
					* (levelHeight / ySplit);
